...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32)  /* TETHER */
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,8,0)
#define GETNAME(pathname) __getname()
#define PUTNAME(name)     __putname(name)
#else
#define GETNAME(pathname) getname(pathname)
#define PUTNAME(name)     putname(name)
#endif
...
/* Macros got undefined in >2.6.16 */
...
extern long			lis_runq_pids[LIS_NR_CPUS] ;
...
#endif
#endif
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,0,0) /* TETHER */
static DEFINE_MUTEX(ioctl_mutex);
long lis_unlocked_ioctl(struct file *fp, unsigned int cmd, unsigned long arg);
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,0,0) /* TETHER */
    unlocked_ioctl: lis_unlocked_ioctl,    /* backward compatible */
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,0,0)  /* TETHER */
static struct dentry *lis_fs_mount(struct file_system_type *fs_type,
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,8,0)
		   mm, MNT_COUNT(mm), mm->mnt_root->d_iname,
#else
		    ...
		    const char *file, int line, const char *fn)
{
	...
	printk("lis_mntput(m@0x%p/%d%s) \"%s\"%s {%s@%d,%s()}\n",
	       m,
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,8,0)
	       MNT_COUNT(m), (MNT_COUNT(m)>0?"--":""), m->mnt_root->d_iname,
#else
...
#if defined(KERNEL_2_5)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32)  /* TETHER */
int lis_super_statfs(struct dentry *d, struct kstatfs *stat) ;
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,0,93)  /* TETHER */
...
void lis_show_inode_aliases( struct inode *i )
{
     ...
     */
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,8,0)
    for (ent = *(i->i_dentry.first)->pprev;  ent != i->i_dentry.first;  ent = *ent->pprev) {
#else
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,8,0)
	  data->mount = file->f_path.mnt = MNTGET(lis_mnt);
#else
...
static struct dentry *lis_fs_mount(struct file_system_type *fs_type,
                         int flags, const char *dev_name, void *data)
{
	return mount_nodev(fs_type, flags, data, lis_fs_setup_sb);
}
#endif
...
#else
    sb = get_sb_nodev(fs_type, flags, ptr, lis_fs_setup_sb, mnt);
#endif
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32)  /* TETHER */
	creds.cr_uid  = current->cred->euid;
	creds.cr_gid  = current->cred->egid;
	creds.cr_ruid = current->cred->uid;
	creds.cr_rgid = current->cred->gid;
#else
...
int	lis_new_file_name_dev(struct file *f, const char *name, dev_t dev)
{
     ...
     */
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,8,0)
    f->f_path.mnt = MNTGET(lis_mnt);	/* (re)mount on LiS */
#else
...
lis_new_inode( struct file *f, dev_t dev )
{
	 ...
	 */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32)  /* TETHER */
	new->i_uid   = current->cred->fsuid;
	new->i_gid   = current->cred->fsgid;
#else
 ...
 */
int lis_ioc_fdetach( char *pathname )
{
    const char* const tmp = GETNAME(pathname);
	...
	if (strcmp( tmp, "*" ) == 0)
	    ...
	    error = lis_fdetach(pathname);  /* arg must be in user space */
	PUTNAME(tmp);
    }
...
static lis_fattach_t *lis_fattach_new(struct file *f, const char *pathname)
{
	...
	if (!error) {
	    data->path = d_path(&path, tmp, PATH_MAX);
#else
...
int lis_sendfd( stdata_t *sendhd, unsigned int fd, struct file *fp )
{
	...
	if (FILE_MNT(fp))
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,8,0)
	    dfp->f_path.mnt = MNTGET(FILE_MNT(fp));
#else
    ...
    sendfd->f.fp  = fp;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
    sendfd->uid   = current->cred->euid.val;
    sendfd->uid   = current->cred->euid.val;
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32)  /* TETHER */
    sendfd->uid   = current->cred->euid;
    sendfd->uid   = current->cred->euid;
#else
...
long lis_unlocked_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
    ...
    struct inode *inode = (fp!=NULL && fp->f_dentry!=NULL) ? fp->f_dentry->d_inode : NULL;
    ret = lis_strioctl(inode, fp, cmd, arg);
...
long lis_compat_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
  ...
  case I_LIS_GETMSG:
  case I_LIS_PUTMSG:
    inode = (fp!=NULL && fp->f_dentry!=NULL) ? fp->f_dentry->d_inode : NULL;
    return lis_strioctl(inode, fp, cmd, arg);
...
#endif  /* defined(_S390X_LIS_) || defined(_PPC64_LIS_) || defined(_X86_64_LIS_) */
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32)  /* TETHER */
    //current->cred->uid = 0 ;			/* become root */
    //current->cred->euid = 0 ;			/* become root */
#else
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,8,0)
    tsk = kthread_run(lis_thread_func, (void *) argp, "LiS-Thread");
    if (IS_ERR(tsk)) {
	return(-ENOMEM) ;
    } else {
...
void	lis_start_qsched(void)
{
	...
	lis_runq_pids[cpu] = lis_thread_start(lis_thread_runqueues,
							  (void *)cpu, name) ;
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,8,0)
	if (lis_runq_pids[cpu] == -ENOMEM)	/* failed to fork */
#else
	if (lis_runq_pids[cpu] < 0)		/* failed to fork */
#endif
	{
	    printk("lis_start_qsched: %s: lis_thread_start error %lu\n",
...
#if defined(KERNEL_2_5)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32)  /* TETHER */
//    ret = sys_mknod(name, mode, new_encode_dev(dev)) ;
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
	if (!error && (current->cred->euid.val != inode->i_uid.val))
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32)  /* TETHER */
	if (!error && (current->cred->euid != inode->i_uid))
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32)  /* TETHER */
	if (!cap_raise(*(kernel_cap_t *)&current->cred->cap_effective, CAP_SYS_ADMIN))
#else
...
#if (!defined(_X86_64_LIS_))
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32)  /* TETHER */
//	    ret = sys_mount(dev_name, dir_name, fstype, rwflag, data) ;
#else
	    ret = syscall_mount(dev_name, dir_name, fstype, rwflag, data) ;
#endif
#endif
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32)  /* TETHER */
	cap_lower(*(kernel_cap_t *)&current->cred->cap_effective, CAP_SYS_ADMIN);
#else
