...
#ifdef KERNEL_2_5
module_init(ipdlpi_init_module);
module_exit(ipdlpi_cleanup_module);
#endif
 ...
 */
void ip2xinet_hw_tx(unsigned char *buf, int len, struct net_device *dev)
{
    /*
...
int ip2xinet_tx(struct sk_buff *skb, struct net_device *dev)
{
    ...
    jiffies = dev_trans_start(dev); /* save the timestamp */
