...
WHOAMI=$(shell whoami)
KERNELMINOR = $(shell uname -r | cut -b5-6)
KERNELMAJOR = $(shell uname -r | cut -b1)
...
#
# If building on 64-bit s390x or ppc64 or x86_64 then build libc for 32-bit 
...
ifeq ($(KERNELMAJOR),3)
   ...
   ifeq ($(KERNELMINOR),32)
	...
	$(Q)cat $(SC_CONFMOD) | generate-modprobe.conf --stdin	>> $@
   endif
endif
